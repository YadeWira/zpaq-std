#   zpaq-std.pod - zpaq-std 58.8 man page source
#
#   Copyright
#
#       Released to public domain by Franco Corbelli & Yade Bravo in 2023
#
#   Description
#
...
zpaq-std - Swiss army knife for backup and disaster recovery

=head1 SYNOPSIS

zpaq-std I<command> I<archive>[C<.zpaq>] [I<files|directory>]... [-I<switches>]...

=head1 DESCRIPTION

I<zpaq-std> is like 7z or RAR on steroids, with versions/"snapshots"
...
The help is embedded : please use that or read the wiki on github
 ...
 zpaq-std           : short version
 zpaq-std h         : list of commands and switches
 zpaq-std h h       : help on everything (ALL IN)
 Help     on XXX     : zpaq-std h   XXX
 Examples of XXX     : zpaq-std -he XXX
...
=head1 OPTIONS
 ...
 zpaq-std h main    : main switches
 zpaq-std h franz   : zpaq-std's switches
 zpaq-std h normal  : 2nd line switches
 zpaq-std h voodoo  : nerd things
...
=head1 AVAILABILITY
 ...
 https://github.com/fcorbelli/zpaq-std
 https://sourceforge.net/projects/zpaq-std/
...
=head1 LIMITATIONS
 ...
 zpaq-std currently does not store owner/group (it will be in the future).
...
=head1 FEATURES
 ...
 zpaq-std supports -stdin and -stdout (zpaq does not)
...
made by Franco Corbelli & Yade Bravo and released under MIT in 2021-2023.
...
=head1 EXAMPLE 1 create a temporary file, add /etc to mybackup.zpaq
 ...
 zpaq-std a /tmp/mybackup.zpaq /etc 
...
=head1 EXAMPLE 3 show archive info (two versions/"snapshots", 1 and 2)
 ...
 zpaq-std i /tmp/mybackup.zpaq

=head1 EXAMPLE 4 list the two different versions (look at different file size)

 zpaq-std l /tmp/mybackup.zpaq -find testfile.txt -until 1
 zpaq-std l /tmp/mybackup.zpaq -find testfile.txt -until 2

=head1 EXAMPLE 5 restore the first version of /etc/testfile.txt (aka: rollback to "snapshot" #1)

 zpaq-std x /tmp/mybackup.zpaq /etc/testfile.txt -to /tmp/restoredfolder/the_first.txt -until 1 -space
