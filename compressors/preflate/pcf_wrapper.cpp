...
/* packPNG (-sa PNG/APNG WebP-lossless recompression) was integrated then removed
   ...
   threading ABI mismatch; see git history around commit 8a5842a). The PCF segment
   kind 5 (PCF_SEG_PACKPNG) stays RESERVED so a future re-add reuses the number and
   does not collide with an already-shipped format value. */
...
bool pcf_file_encode(const std::vector<unsigned char>& original,
  ...
  /* PNG/APNG: intentionally NOT recompressed. packPNG (WebP-lossless) was the
...
int pcf_authentic_reverse2(const std::vector<unsigned char>& stored,
  ...
  if (!pcf_file_decode(stored, orig)) return 1;   /* magic but does not decode: a
      ...
      or an unsupported/reserved segment kind (e.g. a pre-release packPNG archive) */
